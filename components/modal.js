      ...
      backdrop.innerHTML = `
        <section class="modal-card ${config.size === "large" ? "modal-large" : ""}" role="dialog" aria-modal="true" aria-labelledby="modal-title">
          <header class="modal-header">
            <h2 class="modal-title" id="modal-title">${app.helpers.escapeHtml(config.title || "RetailOps")}</h2>
    ...
    confirm(options) {
      return new Promise((resolve) => {
        app.modal.open({
          ...
          content: `<p class="section-copy">${app.helpers.escapeHtml(options.message || "Are you sure?")}</p>`,
          actions: [
            {
